...
class Proc:
    ...
    def __init__(self, func: Callable, args: dict[Any, Any]|None = None, source: str|None = None) -> None:
        self.func = func
        self.args = args or {}
        self.source = source
    ...
    @staticmethod
    def _from(code: str):
        the_code = compile(code, '<string>', 'exec')
        item = None
        for item in the_code.co_consts:
            if hasattr(item, "co_name") and item.co_name == "processor":
                break
        if item is None:
            raise ValueError("Function was not found. Is it called 'processor'?")
        return types.FunctionType(item, globals())
...
op_data = [['GeneratorExp', '', 1], ['Assign', '', 3], ['AnnAssign', '', 5], 
          ['AugAssign', '', 5], ['Expr', '', 5], ['IsNot', 'is not', 5], 
          ['YieldFrom', '', 5], ['If', '', 7], ['For', '', 7], ['Num', '', 9], 
          ['Await', '', 11], ['While', '', 11], ['comprehension_target', '', 13], 
          ['Gt', '>', 13], ['Raise', '', 13], ['Or', 'or', 15], ['Is', 'is', 15], 
          ['ExtSlice', '', 17], ['AsyncFor', '', 17], ['FormattedValue', '', 17], 
          ['comprehension', '', 19], ['Subscript', '', 19], ['NamedExpr', '', 21], 
          ['Assert', '', 21], ['Tuple', '', 21], ['Mod', '%', 21], ['Lt', '<', 21], 
          ['call_one_arg', '', 23], ['RShift', '>>', 23], ['LShift', '<<', 25], 
          ['Pow', '**', 27], ['Div', '/', 27], ['Slice', '', 29], ['Sub', '-', 29], 
          ['Add', '+', 31], ['In', 'in', 31], ['Not', 'not', 33], ['Yield', '', 35], 
          ['Mult', '*', 37], ['Eq', '==', 39], ['Index', '', 41], ['NotEq', '!=', 41], 
          ['LtE', '<=', 41], ['Comma', '', 43], ['NotIn', 'not in', 43], 
          ['BitOr', '|', 45], ['IfExp', '', 45], ['BitXor', '^', 47], 
          ['BitAnd', '&', 49], ['Lambda', '', 51], ['GtE', '>=', 51], 
          ['Return', '', 53], ['And', 'and', 55], ['FloorDiv', '//', 55], 
          ['MatMult', '@', 55], ['PowRHS', '', 57], ['Invert', '~', 59], 
          ['UAdd', '+', 59], ['USub', '-', 59], ['Constant', '', 61]]
...
def precedence_setter():
    ...
    def set_precedence(value, *nodes):
        ...
        if isinstance(value, ast.AST):
            value = precedence_data[type(value)]
        for node in nodes:
...
def _properly_indented(s: str, line_indent: int):
    mylist = s.split('\n')[1:]
...
class Delimit(object):
    ...
    def __init__(self, tree, *args):
        ...
        if node is not None:
            self.p = p = precedence_data[type(op or node)]
...
class Precedence(object):
    vars().update((x, z) for x, _, z in op_data)
...
class SourceGenerator(ExplicitNodeVisitor):
    ...
    def visit_Yield(self, node):
        with self.delimit(node):
            set_precedence(precedence_data[type(node)] + 1, node.value)
            self.write('yield')
            self.conditional_write(' ', node.value)
...
def to_source(source, indent_with=' ' * 4, add_line_information=False):
