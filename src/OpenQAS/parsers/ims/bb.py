...
from OpenQAS.enums import ShuffleType
...
if TYPE_CHECKING:
    ...
    from ...question import _QHasOptions, Question


class Pool:

    def __init__(self, package: _BBExporter, cat: Category, instructions=""):
    ...
    def _question(self, qst: Question, qtype: str, partialcredit='false',
...
class BBImporter(qti1v2.QTIParser1v2):
    """
    """
    ...
    def read(self, item: et.Element, ns: dict):
        ...
        if itemid:
            self._order.append(itemid)
            self._data[itemid] = {}
            itemtitle = item.attrib.get('title')
            if itemtitle:
                self._data[itemid]['title'] = itemtitle
            self._read_item_metadata(item, self._data[itemid], ns)
            self._read_presentation(item, self._data[itemid])
            self._read_processingInfo(item, self._data[itemid])
    ...
    def _read_item_metadata(self, item: et.Element, data: dict, ns: dict):
        ...
        for m in item.findall('./qtimetadata/qtimetadatafield', ns):
            flabel = m.find('fieldlabel')
            if flabel:
                label = self._get_text_value(flabel)
                fentry = m.find('fieldentry')
                if fentry:
...
def read_bb8(cls: Category, filename: str) -> None:
    """_summary_
    ...
    """
    parser = BBImporter()
    tmp = imscc.CC(filename, parser.read, cls)
    tmp.read()
