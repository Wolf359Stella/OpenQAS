...
class _Reader:
    ...
    def _next(self, comp: list[str]) -> str:
        start = self._pos
        self._pos += 1
    ...
    def _handle_item(self):
        all_equals = True
        options: list[tuple[int, str, FText|None]] = []
    ...
    def _set_value_tolerance(self, mtype: str, val: str|float, tol: str|float):
        _tol = float(tol)
        if mtype == "..":
            val = (float(val) + tol)/2
            tol = val - tol
    ...
    def reset(self, data: str, attrs: dict[str, str]) -> None:
        self._pos = 0
        self._scp = False
        self._str = data
        self._attr = attrs
...
def read_gift(cls: type[Category], file_path: str, comment: str|None=None) -> "Category":
    """
    """
    top_quiz = cls()
    quiz = top_quiz
    ...
    with open(file_path, "r", encoding="utf-8") as ifile:
        for line in ifile:
            ...
            if not tmp or tmp[:2] == "//":
                attrs.clear()
                if not (matchs := re.findall(r"\[(.+?)\]", tmp)):
                    continue
                for match in matchs:  # pyright: ignore[reportAny]
                    if match[:5] == "tags:":
...
def write_gift(self: Category, file_path: str) -> None:  # pyright: ignore[reportUnusedParameter]
    """_summary_
