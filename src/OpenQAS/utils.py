...
from .enums import Distribution, FileAddr, TestStatus, TextFormat
...
def _escape_cdata(data: str|None):
    if data is None:
        return ""
...
def _escape_attrib_html(data: str|None):
    if data is None:
        return ""
    if isinstance(data, (int, float)):
        return str(data)
...
class Dataset:
    ...
    def __init__(self, status: TestStatus, name: str, ctype: str,
    ...
    def __eq__(self, __o: object) -> bool:
        if not isinstance(__o, self.__class__):
            return False
        if TestStatus.PRV in (self.status, __o.status):
            return False
