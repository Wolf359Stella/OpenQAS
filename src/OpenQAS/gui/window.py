...
from .layout import AutoUpdateVBox, GCollapsible, GOptions
...
class Editor(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.path: tuple|None = None
        self.top_quiz = Category()
        self.cxt_menu = QMenu(self)
        self.cxt_item: QStandardItem = None
        self.cxt_data: Question | Category = None
        self.cur_question: Question = None
        self.tagbar: GTagBar|None = None
        self.main_editor: GTextEditor|None = None
        self.is_open_find = self.is_open_dataset = False

        with resources.open_text("OpenQAS.gui", "stylesheet.css") as ifile:
    ...
    def _dataview_dropevent(self, event: QDropEvent):
        ...
        if isinstance(to_obj, Category):
            if isinstance(from_obj, Question):
    ...
    def _delete_item(self, *_):
        ...
        if isinstance(self.cxt_data, Question):
    ...
    @staticmethod
    def _new_item(data: Category | Question, parent: QStandardItem):
        name = f"{data.__class__.__name__}_icon.png".lower()
        item = None
        with resources.path("OpenQAS.images", name) as path:
